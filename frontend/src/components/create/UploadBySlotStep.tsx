  ...
  return (
    <div className="flex-1 overflow-y-auto scrollbar-thin p-6">
      <div className="max-w-2xl mx-auto">
        ...
        <p className="text-sm text-[#888888] mt-1">Match each clip to its scene</p>
        ...
        <div className="mt-6 space-y-3">
          {scenes.map((scene: Scene) => {
            ...
            return (
              <div
                key={scene.order}
                className="bg-[#242424] border border-[#333333] rounded-xl p-4"
              >
                <div className="flex items-center gap-3">
                  <span className="w-6 h-6 rounded-full bg-[#333333] text-[#EFEFEF] text-xs font-medium flex items-center justify-center shrink-0">
                    {scene.order}
                  </span>
                ...
                {!uploaded ? (
                  <div
                    ...
                    className="mt-3 border border-dashed border-[#444] rounded-lg p-4 text-center cursor-pointer hover:bg-[#1a1a1a] transition-colors"
                  >
                    <Upload className="w-5 h-5 text-[#888888] mx-auto" />
                    <p className="text-sm text-[#888888] mt-2">
                      Drop clip here or tap to upload
                    </p>
                  </div>
                ) : (
                  <div className="mt-3 flex items-center gap-3 bg-[#1a1a1a] rounded-lg p-3">
                    <div className="w-16 h-10 bg-[#333333] rounded flex items-center justify-center shrink-0">
                    ...
                    <button
                      type="button"
                      onClick={() => fileInputRefs.current[scene.order]?.click()}
                      className="text-xs text-[#888888] underline shrink-0 hover:text-[#EFEFEF]"
                    >
                      Replace
                    </button>
                  </div>
        ...
        <section className="mt-8">
          ...
          <p className="text-xs text-[#888888] mt-1 mb-3">
          ...
          {!audioFile ? (
            <div
              ...
              className="border border-dashed border-[#444] rounded-lg p-4 text-center cursor-pointer hover:bg-[#1a1a1a] transition-colors"
            >
              <Music className="w-5 h-5 text-[#888888] mx-auto" />
              <p className="text-sm text-[#888888] mt-2">Upload background music</p>
            </div>
          ) : (
            <div className="bg-[#242424] border border-[#333333] rounded-lg p-3 flex items-center gap-3">
              <Music className="w-4 h-4 text-[#888888]" />
