...
import { Check, ChevronLeft, X, Sparkles, Cpu } from "lucide-react";
...
export function RenderStep({
  ...
  const opLabels = ["Cutting", "Splicing", "Color grade", "Syncing audio", "Captions"];
  ...
  return (
    <div className="w-full p-4 sm:p-6 pb-28 md:pb-6">
      ...
      <div className={`${isDone ? "max-w-4xl" : "max-w-2xl"} mx-auto`}>
        {!isDone && !isError && (
          <div
            className={`relative overflow-hidden rounded-xl border p-5 sm:p-8 ${
              isPro
                ? "bg-[#0A1012] border-[#51E0CF]/30"
                : "bg-[#0D1416] border-[#152226]"
            }`}
          >
            <div
              className="pointer-events-none absolute left-1/2 top-1/2 h-[360px] w-[440px] -translate-x-1/2 -translate-y-1/2 rounded-full blur-3xl"
              style={{
                background: isPro
                  ? "radial-gradient(ellipse, rgba(81,224,207,0.08) 0%, transparent 70%)"
            ...
            {(() => {
              ...
              const accent = isPro ? "#51E0CF" : "#10B981";
              const clipTracks = [
                {
                  label: "V1",
                  clips: [
                    { start: 0, w: 22, color: accent, delay: 0 },
                    { start: 24, w: 18, color: isPro ? "#8B5CF6" : "#14B8A6", delay: 0.15 },
                    { start: 44, w: 24, color: accent, delay: 0.3 },
                    { start: 70, w: 28, color: isPro ? "#059669" : "#059669", delay: 0.45 },
                  ],
                },
                {
                  label: "V2",
                  clips: [
                    { start: 6, w: 20, color: isPro ? "#8B5CF6" : "#06B6D4", delay: 0.1 },
                    { start: 30, w: 26, color: accent, delay: 0.28 },
                    { start: 60, w: 36, color: isPro ? "#a78bfa" : "#0891B2", delay: 0.42 },
                  ],
                },
              ];
              const waveBars = Array.from({ length: 28 }, (_, i) => {
                const x = i / 27;
                return 0.25 + 0.75 * Math.abs(Math.sin(x * Math.PI * 3.2 + 0.8));
              });

              return (
                <div className="relative flex flex-col items-center text-center px-1 sm:px-4 py-2">
                  <div
                    className={`mb-5 inline-flex items-center gap-2 px-3 py-1 rounded-full border text-xs font-mono font-semibold tracking-wider ${
                      isPro
                        ? "border-[#51E0CF]/30 bg-[#51E0CF]/10 text-[#51E0CF]"
                        : "border-[#10B981]/20 bg-[#10B981]/5 text-[#10B981]"
                    }`}
                  >
                    {isPro ? <Sparkles className="w-3.5 h-3.5" /> : <Cpu className="w-3.5 h-3.5" />}
                    {isPro ? "Pro render" : "Rendering"}
                  </div>

                  {/* Timeline card — same vibe as before, fewer tracks / less chrome */}
                  <div
                    className="relative w-full max-w-[480px] rounded-xl overflow-hidden border"
                    style={{
                      borderColor: isPro ? "rgba(81,224,207,0.28)" : "#1E2D32",
                      background: "#080E10",
                  ...
                  >
                    <div
                      ...
                      style={{ borderColor: "#152226", background: "#0A1214" }}
                    >
                      <div className="flex gap-1.5">
                        <span className="w-[8px] h-[8px] rounded-full bg-[#EF4444]/80" />
                        <span className="w-[8px] h-[8px] rounded-full bg-[#F59E0B]/80" />
                        <span className="w-[8px] h-[8px] rounded-full bg-[#10B981]/80" />
                      </div>
                      <span className="text-[9px] font-mono text-[#3A4A50] ml-2 truncate">
                        Clipr Timeline — {title}
                      </span>
                    ...
                    <div className="px-3 pt-3 pb-1">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-[8px] font-mono text-[#3A4A50] uppercase tracking-wider">
                          Preview
                        </span>
                        <motion.span
                          key={opIdx}
                      ...
                      <div className="flex gap-[2px] h-[28px] overflow-hidden rounded-md">
                          ...
                          <motion.div
                            ...
                            style={{
                              background: `linear-gradient(145deg, ${
                                i % 2 === 0 ? `${accent}40` : isPro ? "rgba(139,92,246,0.28)" : "rgba(6,182,212,0.25)"
                              } 0%, rgba(8,14,16,0.9) 85%)`,
                            }}
                            animate={{
                              opacity: (i / 12) * 100 < pct ? [0.55, 1, 0.7] : 0.2,
                            }}
                    ...
                    <div className="relative mx-3 mt-2 h-3 border-b" style={{ borderColor: "#152226" }}>
                      {[0, 25, 50, 75, 100].map((t) => (
                        <div
                          ...
                          style={{ left: `${t}%` }}
                        >
                          <div className="w-px h-1.5 bg-[#3A4A50]" />
                          <span className="text-[7px] font-mono text-[#3A4A50] mt-0.5 leading-none">
                            {`0:${String(Math.round(t * 0.3)).padStart(2, "0")}`}
                          </span>
                        </div>
                      ))}
                    </div>

                    <div className="relative mx-3 mt-2 mb-1 space-y-[4px]">
                      {clipTracks.map((track, ti) => (
                        <div key={track.label} className="relative flex items-center gap-1.5">
                          <span
                            ...
                            style={{ color: ti === 0 ? accent : isPro ? "#a78bfa" : "#06B6D4" }}
                          >
                            {track.label}
                          </span>
                          <div
                            className="relative flex-1 h-[20px] rounded-[3px] overflow-hidden"
                            ...
                            {track.clips.map((clip, ci) => (
                              <motion.div
                                ...
                                style={{
                                  left: `${clip.start}%`,
                                  width: `${clip.w}%`,
                                  background: `linear-gradient(90deg, ${clip.color}55 0%, ${clip.color}33 100%)`,
                      ...
                      <div className="relative flex items-center gap-1.5">
                        <span className="text-[8px] font-mono font-bold shrink-0 w-5 text-right text-[#F59E0B]">
                          A1
                        </span>
                        <div
                          className="relative flex-1 h-[16px] rounded-[3px] overflow-hidden flex items-end gap-px px-[2px]"
                          style={{ background: "#0B1315" }}
                        >
                          {waveBars.map((h, i) => (
                            <motion.div
                              ...
                              animate={{ opacity: (i / waveBars.length) * 100 < pct ? [0.65, 1, 0.7] : 0.25 }}
                              transition={{ duration: 1.1, repeat: Infinity, delay: i * 0.02 }}
                            />
                          ))}
                        </div>
                      </div>

                      {/* Playhead only — no cut markers */}
                      <div
                        className="absolute top-0 bottom-0 z-20 pointer-events-none"
                        style={{ left: "25px", right: 0 }}
                      >
                        <div className="absolute top-0 bottom-0" style={{ left: `${pct}%` }}>
                          <div
                            className="absolute left-0 top-0 bottom-0 w-[2px] bg-[#EF4444]"
                            style={{ boxShadow: "0 0 8px rgba(239,68,68,0.45)" }}
                          />
                          <div
                            className="absolute left-[-4px] top-[-2px] w-[10px] h-[6px] rounded-b-sm bg-[#EF4444]"
                          />
                        </div>
                      </div>
                    </div>
                    ...
                    >
                      ...
                      <span className="text-[8px] font-mono tabular-nums" style={{ color: accent }}>
                        {Math.round(pct)}%
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="mt-2 flex w-full max-w-[340px] justify-between font-mono text-[9px] tabular-nums text-[#6B7C85]">
                    {["0:00", "0:15", "0:30"].map((t) => (
                      <span key={t}>{t}</span>
                    ))}
                  </div>
                  <p className="mt-1 font-mono text-xs tabular-nums text-[#6B7C85]">
                    {Math.round(pct)}%
                  </p>
                </div>
              );
